package MartinByZuo.数组.滑动窗口;

// 替换子串得到平衡字符串
// 有一个只含有 'Q', 'W', 'E', 'R' 四种字符，且长度为 n 的字符串。
// 假如在该字符串中，这四个字符都恰好出现 n/4 次，那么它就是一个「平衡字符串」。
// 给你一个这样的字符串 s，请通过「替换一个子串」的方式，使原字符串 s 变成一个「平衡字符串」。
// 你可以用和「待替换子串」长度相同的 任何 其他字符串来完成替换。
// 请返回待替换子串的最小可能长度。
// 如果原字符串自身就是一个平衡字符串，则返回 0。
// 测试链接 : https://leetcode.cn/problems/replace-the-substring-for-balanced-string/
public class Code05_ReplaceTheSubstringForBalancedString {

    // ASCII码对应：Q：113 W：119 E：101 R：114
    public static int balancedString(String s) {
        int ans = Integer.MAX_VALUE;
        int need = s.length() / 4;
        // 滑动窗口以外的，QWER出现的次数
        int[] cnt = new int['X'];
        for (int i = 0; i < s.length(); i++) {
            cnt[s.charAt(i)]++;
        }
        for (int left = 0, right = -1; right < s.length(); right++) {
            if (right != -1) {
                --cnt[s.charAt(right)];
            }
            while (left < s.length() && cnt['Q'] <= need && cnt['W'] <= need && cnt['E'] <= need && cnt['R'] <= need) {
                ans = Math.min(ans, right - left + 1);
                ++cnt[s.charAt(left++)];
            }
        }
        return ans;
    }

    public static void main(String[] args) {
        System.out.println(balancedString("QWER"));
        System.out.println(balancedString("QQWR"));
        System.out.println(balancedString("QQQW"));
//        System.out.println(balancedString("RQRERREWEEWWQWRRRWQQEQQQ")); //
//        System.out.println(balancedString("WWQQRRRRQRQQ"));  //
    }


}
